...
#adding a row for STEER did not float with value of 0
siteLARA <- rbind(
  siteLARA,
  data.frame(SiteID = "STEER", count = 0, Float = "N"))
...
npropsAVGE<-c("n=87","n=30","n=80","n=38","n=30","n=89")
...
npropsLARA<-c("n=1","n=117","n=119","n=4","n=104", "n=0")
...
#             N         Y
#Compass [1,]  2.712801 -3.038794
#Steer [2,]  1.768784 -1.981335
#Vessup [3,] -4.451244  4.986143

rownames(chiframe) <- chiframe$SiteID

#looking into running chi squared post hoc test
chiframet<-as.table(as.matrix(chiframe[,-1]))
df<-data_frame(chiframet)

#this doesn't look correct, need to look into this more
chisq.posthoc.test(chiframet,
                   method = "bonferroni")
```
...
rerunning all combinations of LARA logistic regression
...
#flaot length
#exclude site not significant (Best Model!)
NoSLengthLogit<-glm(sprouted~Float+Treatment+PropaguleLength, data=LARA, family="binomial")
summary(NoSLengthLogit)
#282.17


#float, width
NoSWidthLogit<-glm(sprouted~Float+Treatment+`Width-mm`, data=LARA, family="binomial")
summary(NoSWidthLogit)
#278.27

#flaotmass
NoSMassLogit<-glm(sprouted~Float+Treatment+Mass, data=LARA, family="binomial")
summary(NoSMassLogit)
#278.65


#testing assumptions of bestfit model
qqnorm(residuals(NoSMassLogit, type = "pearson"))

#site/float
BasicLogit<-glm(sprouted~SiteID+Float+Treatment, data=LARA, family="binomial")
summary(BasicLogit)
#285.43

#site/float/length

LengthLogit<-glm(sprouted~SiteID+Float+Treatment+PropaguleLength, data=LARA, family="binomial")
summary(LengthLogit)
# 283.92

#site/Float/width
WidthLogit<-glm(sprouted~SiteID+Float+Treatment+`Width-mm`, data=LARA, family="binomial")
summary(WidthLogit)
#280.89

#site/Float/Mass
masslogit<-glm(sprouted~SiteID+Float+Treatment+Mass, data=LARA, family="binomial")
summary(masslogit)
#279.14
```
