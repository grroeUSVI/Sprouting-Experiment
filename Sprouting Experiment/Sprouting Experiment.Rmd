...
going to have to decide if I use the condition column, how I want to split it has these unique values "H""N" "P" "G" "N, P" "N, G" "P, N" "G, N" "N,G"
...
now looking at float/ no float by site 
...
#making a nice version of this plot, how many floated/sank from each site
AVGEFbar1<-ggplot(data=summary_counts_site, aes(x=SiteID, y=count, fill=Float))+
  geom_col(position="dodge")+
  ggtitle(expression(italic("A. germinans") ~ "Propagule Bouyancy Test"))+
  xlab("Propagule Count")+
  ylab("Collection Site")+
...
LV<-expression(italic("L. racemosa")~"Propagules Collected From Vessup Salt Pond")
AV<-expression(italic("A. germinans")~"Propagules Collected From Vessup Salt Pond")
LC<-expression(italic("L. racemosa")~"Propagules Collected From Compass Point")
AC<-expression(italic("A. germinans")~"Propagules Collected From Compass Point")
LS<-expression(italic("L. racemosa")~"Propagules Collected From STEER")
...
look at all the props together not including site
...
Total sprouted each treatment
```{r}
View(TreatmentSprout)

TreatmentSprout<-GermSproutCount

TreatmentSprout<-as.data.frame(TreatmentSprout)
TreatmentSprout<-as.factor(TreatmentSprout$Species)
TreatmentSprout<-as.factor(TreatmentSprout$Treatment)

TreatmentSprout<-TreatmentSprout%>%
  group_by(Species,Treatment)%>%
  summarise(totalSp=mutate(sum(TotalSprouted)))
